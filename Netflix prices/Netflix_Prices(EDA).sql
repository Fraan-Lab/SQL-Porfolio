...
/*NUMBER OF COUNTRIES USING EACH CURRENCY TYPE*/
...
SELECT COUNT(Mobile) AS 'Number of Countries'
FROM netflix_prices
WHERE Mobile_USD IN (0) AND Currency='United States Dollar';
...
SELECT 
Country, 
MIN(Mobile_USD) AS 'Mobile Plan Price in USD'
 FROM netflix_prices 
 GROUP BY Country
...
/*TOP 5 COUNTRIES WITH THE HIGHEST PRICE IN USD FOR PREMIUM PLAN*/
 ...
 SELECT 
 Country, 
 MAX(Premium_USD) AS 'Premium Plan Price in USD'
 FROM netflix_prices
 GROUP BY Country
